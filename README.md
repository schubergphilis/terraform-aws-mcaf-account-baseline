...
| <a name="input_aws_ebs_snapshot_block_public_access_state"></a> [aws\_ebs\_snapshot\_block\_public\_access\_state](#input\_aws\_ebs\_snapshot\_block\_public\_access\_state) | Configure regionally the EBS snapshot public sharing policy, alternatives: `block-all-sharing` and `unblocked`. Optionally set to null explicitely when using Declarative Policies at Organization level. | `string` | `"block-new-sharing"` | no |
