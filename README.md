...
| <a name="input_aws_ebs_encryption_custom_key"></a> [aws\_ebs\_encryption\_custom\_key](#input\_aws\_ebs\_encryption\_custom\_key) | Set to true and specify the `aws_kms_key_arns` to use in place of the AWS-managed default CMK | `bool` | `false` | no |
| <a name="input_aws_ebs_snapshot_block_public_access_config"></a> [aws\_ebs\_snapshot\_block\_public\_access\_config](#input\_aws\_ebs\_snapshot\_block\_public\_access\_config) | EBS snapshot block public access configuration. Set enabled to false to opt out when the setting is governed at the AWS Organization level via Declarative Policies. | <pre>object({<br/>    enabled = optional(bool, true)<br/>    state   = optional(string, "block-new-sharing")<br/>  })</pre> | `{}` | no |
| <a name="input_aws_ec2_image_block_public_access_config"></a> [aws\_ec2\_image\_block\_public\_access\_config](#input\_aws\_ec2\_image\_block\_public\_access\_config) | EC2 AMI block public access configuration. Set enabled to false to opt out when the setting is governed at the AWS Organization level via Declarative Policies. | <pre>object({<br/>    enabled = optional(bool, true)<br/>    state   = optional(string, "block-new-sharing")<br/>  })</pre> | `{}` | no |
| <a name="input_aws_kms_key_arns"></a> [aws\_kms\_key\_arns](#input\_aws\_kms\_key\_arns) | Map of region name => KMS Key ARN to use for EBS default encryption (and optionally other resources). Safely attaches/creates KMS resources only when a key for the region exists. | `map(string)` | `{}` | no |
...
| <a name="input_aws_ssm_automation_log_group_name"></a> [aws\_ssm\_automation\_log\_group\_name](#input\_aws\_ssm\_automation\_log\_group\_name) | CloudWatch Logs log group name for SSM Automation. | `string` | `"/aws/ssm/automation"` | no |
| <a name="input_aws_ssm_automation_logging_enabled"></a> [aws\_ssm\_automation\_logging\_enabled](#input\_aws\_ssm\_automation\_logging\_enabled) | When true, enable SSM Automation customer-script logging to CloudWatch Logs in each baseline region. | `bool` | `true` | no |
