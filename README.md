<!-- migrate-repo:banner -->
> [!IMPORTANT]
> **This repository has moved to [`schubergphilis-ep/terraform-aws-mcaf-account-baseline`](https://github.com/schubergphilis-ep/terraform-aws-mcaf-account-baseline).**
...
| <a name="input_aws_ebs_snapshot_block_public_access_state"></a> [aws\_ebs\_snapshot\_block\_public\_access\_state](#input\_aws\_ebs\_snapshot\_block\_public\_access\_state) | Configure regionally the EBS snapshot public sharing policy, alternatives: `block-all-sharing` and `unblocked`. Not `null` but `unblocked` should be chosen to explicitly opt out of regional configuration when the setting is governed at the AWS Organization level via Declarative Policies. | `string` | `"block-new-sharing"` | no |
